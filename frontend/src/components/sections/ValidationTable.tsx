...
import { structuredRequirements } from "@/lib/mockData";
  ...
  return (
    <>
      ...
      >
        ...
        <Spec value={`${r.length_mm}mm`} fail={r.length_mm > structuredRequirements.max_length_mm} />
        <Spec value={`${r.power_watts}W`} fail={r.power_watts > structuredRequirements.max_power_watts} />
        <Spec value={`€${r.price_eur}`} fail={r.price_eur > structuredRequirements.budget_eur} />
        <Spec value={`${r.delivery_days}d`} fail={r.delivery_days > structuredRequirements.max_delivery_days} />
        <Spec
          value={`${r.warranty_years}yr`}
          fail={r.warranty_years < structuredRequirements.minimum_warranty_years}
        />
...
function FailedDrawer({ r }: { r: ValidationResult }) {
  const checks = [
    {
      label: "Length",
      actual: `${r.length_mm}mm`,
      limit: `≤ ${structuredRequirements.max_length_mm}mm`,
      fail: r.length_mm > structuredRequirements.max_length_mm,
    },
    {
      label: "Power draw",
      actual: `${r.power_watts}W`,
      limit: `≤ ${structuredRequirements.max_power_watts}W`,
      fail: r.power_watts > structuredRequirements.max_power_watts,
    },
    {
      label: "Price",
      actual: `€${r.price_eur}`,
      limit: `≤ €${structuredRequirements.budget_eur}`,
      fail: r.price_eur > structuredRequirements.budget_eur,
    },
    {
      label: "Delivery",
      actual: `${r.delivery_days}d`,
      limit: `≤ ${structuredRequirements.max_delivery_days}d`,
      fail: r.delivery_days > structuredRequirements.max_delivery_days,
    },
    {
      label: "Warranty",
      actual: `${r.warranty_years}yr`,
      limit: `≥ ${structuredRequirements.minimum_warranty_years}yr`,
      fail: r.warranty_years < structuredRequirements.minimum_warranty_years,
    },
  ];
